...
#endif
{
  ...
  std::fill(transferFunctionMagnitudes.begin(),
            transferFunctionMagnitudes.end(), 0.0f);
...
int MondoSpectralRefAudioProcessor::getNumPrograms() { return 1; }

int MondoSpectralRefAudioProcessor::getCurrentProgram() { return 0; }
...
void MondoSpectralRefAudioProcessor::changeProgramName(
    int index, const juce::String &newName) {}

void MondoSpectralRefAudioProcessor::prepareToPlay(double sampleRate,
                                                   int samplesPerBlock) {
...
bool MondoSpectralRefAudioProcessor::isBusesLayoutSupported(
...
void MondoSpectralRefAudioProcessor::processBlock(
  ...
  for (int i = 0; i < buffer.getNumSamples(); ++i) {
    ...
    int mode = analyzeChannelMode.load();
    ...
    if (totalNumInputChannels == 1) {
      ...
      if (mode == 1) { // Left
        ...
        returnSample = buffer.getReadPointer(1)[i];
      } else { // Avg (L+R)
        returnSample =
            (buffer.getReadPointer(0)[i] + buffer.getReadPointer(1)[i]) * 0.5f;
      }
    }

    pushNextSampleIntoFifo(noise, returnSample);
    // Output noise to all available output channels
    for (int ch = 0; ch < totalNumOutputChannels; ++ch) {
      buffer.getWritePointer(ch)[i] = noise;
    }
  }
}

void MondoSpectralRefAudioProcessor::pushNextSampleIntoFifo(
    float sendSample, float returnSample) {
  if (fifoIndex == fftSize) {
    if (!nextFFTBlockReady) {
      std::copy(sendFifo.begin(), sendFifo.begin() + fftSize,
                sendFftData.begin());
      std::copy(returnFifo.begin(), returnFifo.begin() + fftSize,
                returnFftData.begin());
...
void MondoSpectralRefAudioProcessor::calculateTransferFunction() {
  if (nextFFTBlockReady) {
    // Sin ventana (Rectangular) porque usamos ruido periódico sincronizado al
    // tamaño de la FFT
    ...
    for (int i = 0; i < fftSize / 2; ++i) {
      ...
      transferFunctionMagnitudes[i] =
          0.8f * transferFunctionMagnitudes[i] + 0.2f * tfMag;
      ...
      if (isNormalized.load() && baselineMagnitudes[i] > 1e-5f) {
        displayMagnitudes[i] =
            transferFunctionMagnitudes[i] / baselineMagnitudes[i];
      } else {
...
bool MondoSpectralRefAudioProcessor::hasEditor() const { return true; }

juce::AudioProcessorEditor *MondoSpectralRefAudioProcessor::createEditor() {
  return new MondoSpectralRefEditor(*this);
}

void MondoSpectralRefAudioProcessor::getStateInformation(
    juce::MemoryBlock &destData) {}

void MondoSpectralRefAudioProcessor::setStateInformation(const void *data,
                                                         int sizeInBytes) {}
