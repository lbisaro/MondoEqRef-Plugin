...
MondoEqRefAudioProcessorEditor::MondoEqRefAudioProcessorEditor(
  ...
  resetButton.onClick = [this] {
    ...
    std::fill(representativeCurveHann.begin(), representativeCurveHann.end(),
              0.0f);
        ...
        if (presetsArray.isArray()) {
          ...
          for (auto &item : *arr) {
            if (item.isObject()) {
              ...
              if (presetObj->hasProperty("lufs"))
                target.targetLufs = float(presetObj->getProperty("lufs"));
              ...
              if (bandsProp.isArray()) {
                ...
                for (auto &bandItem : *bandsArr) {
                  if (bandItem.isObject()) {
                    ...
                    if (bObj->hasProperty("tip"))
                      band.tip = bObj->getProperty("tip").toString();
                    else
                      band.tip = "";
                    band.color = juce::Colour::fromString(
                        bObj->getProperty("color").toString());
          ...
          std::sort(presets.begin(), presets.end(),
                    [](const PresetTarget &a, const PresetTarget &b) {
...
void MondoEqRefAudioProcessorEditor::targetRoleChanged() {
  ...
  if (selectedId == 999) {
    // Show Multi-selection dialog for Stem Refs using native file chooser
    // (Async)
    currentPresetIndex = -1;

    fileChooser = std::make_unique<juce::FileChooser>(
        "Select Stem Refs", audioProcessor.stemDirectory, "*.json");
    fileChooser->launchAsync(
        ...
        [this](const juce::FileChooser &fc) {
          ...
          for (const auto &f : results) {
            ...
            if (parsed.isObject()) {
              auto curveProp =
                  parsed.getDynamicObject()->getProperty("representativeCurve");
...
void MondoEqRefAudioProcessorEditor::updateFftSize() {
  ...
  windowHann = std::make_unique<juce::dsp::WindowingFunction<float>>(
      currentFftSize, juce::dsp::WindowingFunction<float>::hann);
  windowBH = std::make_unique<juce::dsp::WindowingFunction<float>>(
      currentFftSize, juce::dsp::WindowingFunction<float>::blackmanHarris);
...
void MondoEqRefAudioProcessorEditor::pushNextSampleIntoFifo(
...
void MondoEqRefAudioProcessorEditor::drawNextFrameOfSpectrum() {
  ...
  windowHann->multiplyWithWindowingTable(fftDataHann.data(),
                                         (size_t)currentFftSize);
  forwardFFT->performFrequencyOnlyForwardTransform(fftDataHann.data());

  windowBH->multiplyWithWindowingTable(fftDataBH.data(),
                                       (size_t)currentFftSize);
  ...
  if (maxMag > silenceThreshold) {
    ...
    if (juce::Time::getMillisecondCounter() - lastSignalTime >
        (uint32_t)appSettings.autoResetWaitMs) {
      std::fill(representativeCurve.begin(), representativeCurve.end(), 0.0f);
      std::fill(sumCurve.begin(), sumCurve.end(), 0.0f);
      std::fill(representativeCurveHann.begin(), representativeCurveHann.end(),
                0.0f);
      ...
      audioProcessor.resetLufs();
      lastSignalTime =
          juce::Time::getMillisecondCounter(); // Prevent continuous resetting
    }
  }
}
...
void MondoEqRefAudioProcessorEditor::paint(juce::Graphics &g) {
  ...
  auto meterArea =
      fullArea.removeFromRight(100).withTrimmedTop(40).withTrimmedBottom(20);
  auto leftArea =
      fullArea.removeFromLeft(40).withTrimmedTop(40).withTrimmedBottom(20);
  ...
  if (currentPresetIndex >= 0 && currentPresetIndex < presets.size()) {
    ...
    for (int i = 1; i < currentFftSize / 2; ++i) {
      float freq = (float)i * ((float)audioProcessor.getSampleRate() /
                               (float)currentFftSize);
      if (freq >= 100.0f && freq <= 16000.0f) {
        ...
        if (dbLevel > -99.0f) {
          if (dbLevel < globalMinDb)
            globalMinDb = dbLevel;
          if (dbLevel > globalMaxDb)
            globalMaxDb = dbLevel;
        }
      }
    }
    ...
    if (!target.points.empty()) {
      ...
      for (const auto &pt : target.points) {
        if (pt.f < minFreq || pt.f > maxFreq)
          continue;
        float normX =
            (std::log10(pt.f) - minLogFreq) / (maxLogFreq - minLogFreq);
        ...
        float targetY = bottom - juce::jmap(tiltedTarget, minDecibels,
                                            maxDecibels, 0.0f, 1.0f) *
                                     height;
      ...
      if (!smoothTarget.empty()) {
        ...
        for (const auto &pt : target.points) {
          float tiltOffset =
              isTiltEnabled ? 4.5f * std::log2(pt.f / 1000.0f) : 0.0f;
          float tiltedTarget = pt.target + tiltOffset + currentTargetOffset;

          float normFreq =
              (std::log10(pt.f) - minLogFreq) / (maxLogFreq - minLogFreq);
          float px = left + width * std::pow(normFreq, skewFactor);
          float py = bottom - juce::jmap(juce::jlimit(minDecibels, maxDecibels,
                                                      tiltedTarget),
  ...
  if (isStemRefActive && !stemRefAvgCurve.empty()) {
    ...
    float binFreq = (float)audioProcessor.getSampleRate() /
                    (float)(stemRefAvgCurve.size() * 2);
    ...
    for (size_t i = 1; i < stemRefAvgCurve.size(); ++i) {
      ...
      pixelMin[xPixel] =
          juce::jmax(pixelMin[xPixel], mapDb(stemRefMinCurve[i]));
      pixelMax[xPixel] =
          juce::jmax(pixelMax[xPixel], mapDb(stemRefMaxCurve[i]));
      pixelAvg[xPixel] =
          juce::jmax(pixelAvg[xPixel], mapDb(stemRefAvgCurve[i]));
    }
    ...
    for (int x = 0; x <= juce::roundToInt(width); ++x) {
      auto getY = [&](float db) {
        float level = juce::jmap(juce::jlimit(minDecibels, maxDecibels, db),
                                 minDecibels, maxDecibels, 0.0f, 1.0f);
    ...
    firstRef = true;
    for (int x = 0; x <= juce::roundToInt(width); ++x) {
      float yMax = bottom - juce::jmap(juce::jlimit(minDecibels, maxDecibels,
                                                    pixelMax[x]),
                                       minDecibels, maxDecibels, 0.0f, 1.0f) *
                                height;
      if (firstRef) {
        shadedPath.startNewSubPath(left + x, yMax);
        firstRef = false;
      } else
        shadedPath.lineTo(left + x, yMax);
    }
    for (int x = juce::roundToInt(width); x >= 0; --x) {
      float yMin = bottom - juce::jmap(juce::jlimit(minDecibels, maxDecibels,
                                                    pixelMin[x]),
                                       minDecibels, maxDecibels, 0.0f, 1.0f) *
                                height;
  ...
  if (validFrameCount > 0 && !representativeCurve.empty() &&
      !maxPeakCurve.empty()) {
    ...
    newLowCF =
        calcCrestFactor(appSettings.bandLow.hz_min, appSettings.bandLow.hz_max);
    newMidCF =
        calcCrestFactor(appSettings.bandMid.hz_min, appSettings.bandMid.hz_max);
    newHiCF = calcCrestFactor(appSettings.bandHigh.hz_min,
                              appSettings.bandHigh.hz_max);
  }
  ...
  if (validFrameCount > 0) {
    ...
    auto drawCrestBar = [&](juce::Rectangle<int> area,
                            const juce::String &label, float cf,
                            float minTarget, float maxTarget) {
      g.setColour(juce::Colours::white.withAlpha(0.7f));
      g.drawText(label + ": " + juce::String(cf, 1) + " dB",
    ...
    juce::Rectangle<int> r1(crestFactorArea.getX(), crestFactorArea.getY(),
                            (int)colWidth, crestFactorArea.getHeight());
    juce::Rectangle<int> r2((int)(crestFactorArea.getX() + colWidth),
                            crestFactorArea.getY(), (int)colWidth,
                            crestFactorArea.getHeight());
    juce::Rectangle<int> r3((int)(crestFactorArea.getX() + colWidth * 2),
                            crestFactorArea.getY(), (int)colWidth,
                            crestFactorArea.getHeight());

    drawCrestBar(r1, "LOW DYNAMICS", newLowCF, appSettings.bandLow.db_min,
                 appSettings.bandLow.db_max);
    drawCrestBar(r2, "MID DYNAMICS", newMidCF, appSettings.bandMid.db_min,
                 appSettings.bandMid.db_max);
    drawCrestBar(r3, "HI DYNAMICS", newHiCF, appSettings.bandHigh.db_min,
                 appSettings.bandHigh.db_max);
  }
  ...
  g.drawText("LUFS", meterArea.getX(), meterArea.getY() - 10,
             meterArea.getWidth(), 20, juce::Justification::centred, false);

  float intLufs = audioProcessor.getIntegratedLufs();
  float stLufs = audioProcessor.getShortTermLufs();

  // Meter drawing helper
  auto drawMeter = [&](juce::Rectangle<int> area, float lufsValue,
                       juce::String label, bool isIntegrated) {
    // Draw background
    ...
    juce::Rectangle<int> fillRect =
        area.withTrimmedTop(area.getHeight() - fillHeight);
    ...
    if (isIntegrated && currentPresetIndex >= 0 &&
      ...
      float targetRatio =
          (mappedTarget - minDecibels) / (maxDecibels - minDecibels);
      int targetY =
          area.getBottom() - juce::roundToInt(targetRatio * area.getHeight());
      ...
      g.drawText(juce::String(targetLufs, 1), area.getX() - 30, targetY - 6, 28,
                 12, juce::Justification::centredRight, false);
    }
    ...
    g.setColour(juce::Colours::white);
    g.setFont(12.0f);
    g.drawText(label, area.getX(), area.getY() - 15, area.getWidth(), 15,
               juce::Justification::centred, false);

    // Draw value
    juce::String valStr =
        lufsValue > -99.0f ? juce::String(lufsValue, 1) : "--";
    g.drawText(valStr, area.getX() - 5, area.getBottom() + 2,
               area.getWidth() + 10, 15, juce::Justification::centred, false);
  };
  ...
  juce::Rectangle<int> intMeterArea(startX, meterArea.getY() + 25, meterWidth,
                                    meterArea.getHeight() - 45);
  juce::Rectangle<int> stMeterArea(intMeterArea.getRight() + spacing,
                                   meterArea.getY() + 25, meterWidth,
                                   meterArea.getHeight() - 45);
    ...
    if (parsedJson.isObject()) {
      ...
      if (obj->hasProperty("auto_reset")) {
        ...
        if (autoResetProp.isObject()) {
          ...
          if (arObj->hasProperty("threshold"))
            appSettings.autoResetThreshold =
                float(arObj->getProperty("threshold"));
          if (arObj->hasProperty("enabled"))
            appSettings.autoResetEnabled = bool(arObj->getProperty("enabled"));
          if (arObj->hasProperty("wait_ms"))
            appSettings.autoResetWaitMs = int(arObj->getProperty("wait_ms"));

          autoResetButton.setToggleState(appSettings.autoResetEnabled,
      ...
      if (obj->hasProperty("dynamics_meter")) {
        ...
        if (dynamicsProp.isObject()) {
          ...
          auto parseBand = [](juce::DynamicObject *parent,
            ...
            if (parent->hasProperty(bandName)) {
              ...
              if (bandProp.isObject()) {
                ...
                if (bObj->hasProperty("hz_min"))
                  band.hz_min = float(bObj->getProperty("hz_min"));
                if (bObj->hasProperty("hz_max"))
                  band.hz_max = float(bObj->getProperty("hz_max"));
                if (bObj->hasProperty("db_min"))
                  band.db_min = float(bObj->getProperty("db_min"));
                if (bObj->hasProperty("db_max"))
                  band.db_max = float(bObj->getProperty("db_max"));
              }
            }
          };
...
void MondoEqRefAudioProcessorEditor::mouseWheelMove(
...
void MondoEqRefAudioProcessorEditor::mouseDoubleClick(
...
juce::String MondoEqRefAudioProcessorEditor::getTooltip() {
  // Note: getTooltip doesn't have the mouse event, but juce::Desktop can
  // provide the mouse position, or we can use a stored mouse position. However,
  // getTooltip is queried periodically. TooltipClient provides getTooltip() but
  // usually TooltipWindow handles checking which Component is under mouse. If
  // the whole Editor is the TooltipClient, we check the relative mouse pos.
